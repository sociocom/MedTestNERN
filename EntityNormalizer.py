...
class EntityDictionary:
    ...
    def get_normalized_term(self, term):
        return (
            self.df[self.df.iloc[:, self.candidate_column] == term]
            .iloc[:, self.normalization_column]
            .item()
        )
...
class DefaultTest(EntityDictionary):

    def __init__(self):
        super().__init__("dictionaries/test_dict.csv", 0, 2)
