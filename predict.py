...
def to_xml(data, id_to_tags):
    ...
    for i, entities in enumerate(data["entities_predicted"]):
        ...
        if not type_id[1] == "":
            attr = (
                " " + value_to_key(type_id[1], key_attr) + "=" + '"' + type_id[1] + '"'
            )
        else:
            attr = ""
    ...
    for dataset in sentences_list:
        ...
        for sample in tqdm(dataset, desc="Predict", leave=False):
            ...
            text_entities.append(
                {"text": text, "entities_predicted": entities_predicted}
            )
...
def normalize_entities(
    ...
    id_to_tags,
    disease_dict=None,
    disease_candidate_col=None,
    disease_normalization_col=None,
    disease_matching_threshold=None,
    drug_dict=None,
    drug_candidate_col=None,
    drug_normalization_col=None,
    drug_matching_threshold=None,
    test_dict=None,
    test_candidate_col=None,
    test_normalization_col=None,
    test_matching_threshold=None,
):
    if disease_dict:
        disease_dict = EntityDictionary(
            disease_dict, disease_candidate_col, disease_normalization_col
        )
    else:
        disease_dict = DefaultDiseaseDict()
    disease_normalizer = EntityNormalizer(
        disease_dict, matching_threshold=disease_matching_threshold
    )

    if drug_dict:
        drug_dict = EntityDictionary(
            drug_dict, drug_candidate_col, drug_normalization_col
        )
    else:
        drug_dict = DefaultDrugDict()
    drug_normalizer = EntityNormalizer(
        drug_dict, matching_threshold=drug_matching_threshold
    )

    if test_dict:
        test_dict = EntityDictionary(
            test_dict, test_candidate_col, test_normalization_col
        )
    else:
        test_dict = DefaultTest()
    test_normalizer = EntityNormalizer(
        test_dict, matching_threshold=test_matching_threshold
    )
        ...
        for text_entities in entry:
            ...
            for entity in entities:
                ...
                normalizer = (
                    drug_normalizer
                    if tag == "m-key"
                    else disease_normalizer if tag == "d" else None
                )
                normalizer = (
                    test_normalizer
                    if tag == "t-test"
...
def run(model, input, output=None, normalize=False, **kwargs):
    ...
    classification_model = ner.BertForTokenClassification_pl.from_pretrained_bin(
        model_path=model, num_labels=2 * len_num_entity_type + 1
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Enable entity normalization",
        default=False,
    )

    # Dictionary override arguments
    parser.add_argument(
        "--drug-dict", help="File path for overriding the default drug dictionary"
    )
    parser.add_argument(
        "--drug-candidate-col",
        type=int,
        help="Column name for drug candidates in the CSV file (required if --drug-dict is specified)",
    )
    parser.add_argument(
        "--drug-normalization-col",
        type=int,
        help="Column name for drug normalization in the CSV file (required if --drug-dict is specified",
    )
    parser.add_argument(
        "--disease-matching-threshold",
        type=int,
        default=50,
        help="Matching threshold for disease dictionary",
    )

    parser.add_argument(
        "--disease-dict", help="File path for overriding the default disease dictionary"
    )
    parser.add_argument(
        "--disease-candidate-col",
        type=int,
        help="Column name for disease candidates in the CSV file (required if --disease-dict is specified)",
    )
    parser.add_argument(
        "--disease-normalization-col",
        type=int,
        help="Column name for disease normalization in the CSV file (required if --disease-dict is specified)",
    )
    parser.add_argument(
        "--drug-matching-threshold",
        type=int,
        default=50,
        help="Matching threshold for drug dictionary",
    )
    parser.add_argument(
        "--test-dict", help="File path for overriding the default test dictionary"
    )
    parser.add_argument(
        "--test-candidate-col",
        type=int,
        help="Column name for test candidates in the CSV file (required if --test-dict is specified)",
    )
    parser.add_argument(
        "--test-normalization-col",
        type=int,
        help="Column name for test normalization in the CSV file (required if --test-dict is specified)",
    )
    parser.add_argument(
        "--test-matching-threshold",
        type=int,
        default=50,
        help="Matching threshold for test dictionary",
    )
